class Autocomplete {
  ...
  getMatches( text ) {
   const optionsOfInput = this.input.options;
    let result = [];
    for (let i = 0; i < optionsOfInput.length; i ++){
      if (optionsOfInput[i].textContent.includes(text)) {
         result.push({text: optionsOfInput[i].textContent , value: optionsOfInput[i].value})
      } 
    }
    return result;
  }
